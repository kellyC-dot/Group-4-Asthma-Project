...
###*erase this later* 23 Nov Meeting with Dr. Wheeler, we should subset this box plot by Central California Public Health Officer Region, focusing on Calaveras, Fresno, Kern, Kings, Madera, Mariposa, Merced, San Joaquin, Stanislaus, Tulare, Tuolumne. Reference to https://www.cdph.ca.gov/Programs/RPHO
###Action: Eve to implement the subset concept on the above box plot, everything following that will need to be adjusted to conform to focusing only on these counties.

#Filtered Dataset for Central Valley Counties
```{r}

# Define the 20 Central Valley counties
central_valley_20 <- c("Butte", "Colusa", "Fresno", 
                       "Glenn", "Kern", "Kings", 
                       "Madera", "Merced", "Placer", 
                       "Sacramento", "San Joaquin", 
                       "Shasta", "Stanislaus", "Sutter", 
                       "Tehama", "Tulare", "Yolo", 
                       "Yuba", "Mariposa", "Tuolumne")

# Filter the asthma_all_join dataset
asthma_central_valley_all_joined_dataset <- asthma_all_join %>%
  filter(county %in% central_valley_20)

# View the filtered dataset
print(asthma_central_valley_all_joined_dataset)

```
# Spearman Correlation Coefficients Between Median Age-Adjusted ED Visit Rates and Age Groups in Central Valley, 2019
```{r}
...
# Calculate the Spearman correlation for each age group
...
# Rank the age groups by the strength of the correlation (strongest to weakest)
cor_df <- cor_df[order(-cor_df$Spearman_Correlation), ]  # Sort by correlation in descending order
...
datatable(
  ...
  caption = htmltools::tags$caption(
    style = 'caption-side: top; text-align: center; font-weight: bold; font-size: 16px;',
    "Spearman Correlation Coefficients for Median Age-Adjusted ED Visits Rate and Age Groups (Central Valley, 2019)"
  )
)
```
...
###The results show that all age groups exhibit moderate positive correlations between median visits and the overall Median Age-Adjusted ED Visit Rate in the Central Valley in 2019, with children (0–17 years) leading (r = 0.566) compared to seniors (r = 0.449) and adults (r = 0.442). This indicates that while all age groups contribute to asthma-related ED visits, children are more closely associated with the overall rates, highlighting their heightened vulnerability to asthma triggers in the region.
