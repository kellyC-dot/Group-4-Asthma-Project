...
asthma_ed_visits_2019_clean <- asthma_ed_visits_2019_clean %>%
  mutate(
    race_ethnicity = case_when(
      race_ethnicity %in% c("Hispanic", "Latino") ~ "Hispanic",
      race_ethnicity %in% c("White", "Caucasian") ~ "White",
      race_ethnicity %in% c("Black", "African American") ~ "Black",
      race_ethnicity %in% c("Native American", "American Indian", "AI/AN") ~ "Native American",
      race_ethnicity %in% c("Asian", "Asian/PI", "Pacific Islander") ~ "Asian",
      TRUE ~ "Other"  # Catch-all for categories not explicitly listed
    )
  )
...
asthma_ed_join <- asthma_ed_visits_2019_clean %>%
  ...
  summarize(
    ...
    total_hispanic_visits_ds3 = sum(ifelse(race_ethnicity == "Hispanic", number_of_ed_visits, 0), na.rm = TRUE),
    total_white_visits_ds3 = sum(ifelse(race_ethnicity == "White", number_of_ed_visits, 0), na.rm = TRUE),
    total_black_visits_ds3 = sum(ifelse(race_ethnicity == "Black", number_of_ed_visits, 0), na.rm = TRUE),
    total_native_american_visits = sum(ifelse(race_ethnicity == "Native American", number_of_ed_visits, 0), na.rm = TRUE),
    total_asian_visits_ds3 = sum(ifelse(race_ethnicity == "Asian", number_of_ed_visits, 0), na.rm = TRUE),
    total_other_visits_ds3 = sum(ifelse(race_ethnicity == "Other", number_of_ed_visits, 0), na.rm = TRUE),
    
    .groups = "drop"
  ) %>%
  arrange(desc(asthma_median_by_county_ds3))  # Arrange by median ED visits
