...
```
## The CalEnviron Study used 21 indicators to characterize Pollution Burden and Population Characteristics by County. Our study evaluated asthma, which is the Age Adjusted Rate of ED Visits, and its potential association with the select environmental pollution variables PM2.5, diesel PM, traffic, toxic release and the select population characteristic variables housing, education, unemployment and poverty. We found a modest correlation between asthma ED visits and population characteristics, and a weaker correlation with pollution variables as shown in the tables and visuals below. 
...
```
# Based on our discovery that most counties in the Central Valley had relatively high asthma ED visist rates that correlate with a high CES 4.0 CalEnviron Score, we evaluated pollution variables in this subset of counties. Pearson correlation coefficients were higher for this subset of counties compared to all California counties. 

Scatterplots: Correlation between CalEnviron Variables and Median Age Adjusted Asthma ED Visit Rate by Subset of Central Valley Counties
...
ggplot(asthma_long, aes(x = Value, y = asthma_median_by_county_ds1)) +
  ...
  facet_wrap(~ Variable, scales = "free_x") +
  labs(
    title = "Median Asthma ED Visit Rates vs Pollution Variables for Central Valley Counties Subset",
    x = "Variable Value",
    y = "Asthma Median by County"
  ) +
  theme_minimal() +
...
```
# 
#Table: Correlation Coefficients between Pollution Variables and Median Age Adjusted Asthma ED Visit Rate by Central Valley Counties Subset
```{r}
library(DT)
...
```
# We next looked at data on a census level within the counties to understand how asthma ED visits are distributed within the counties. Eight out of the eleven Central Vally counties are among those with the highest median ED visit rate prompting us to further explore this subset.

#Boxplots:  Asthma ED Visit Rates by County Census Tracts. Counties are ordered by median Asthma Rates (ascending).
```{r}
library(ggplot2)
library(dplyr)
library(tidyr)
...
```
# Boxplots: CES score vs Asthma ED Visit Rates by County Census Tracts. Counties are ordered by median Asthma Rates (ascending) to show the correlation between Asthma ED Visit Rates and CES score (0.636)
```{r}
library(dplyr)
library(tidyr)
library(ggplot2)
...
## Scatterplot: Correlation between Median Asthma ED Visit Rates and CES 4.0 Score by County
...
ggplot(asthma_all_join, aes(x = asthma_median_by_county_ds1, y = mean_ces_score)) +
  ...
  labs(
    title = "Median Asthma Rates and CES 4.0 Score by County",
...
## Focus on Central Valley Counties. Eight out of the eleven Central Valley counties rank in the top 20 counties for highest ED visit rates. 
...
```
# Correlation between Median Age Adjsted Asthma ED Visits and Mean CES 4.0 Score for Central Valley Counties Subset
...
```
# Scatterplot: Correlation between Median Asthma ED Visit Rates and CES 4.0 Score for Central Valley County Subset
