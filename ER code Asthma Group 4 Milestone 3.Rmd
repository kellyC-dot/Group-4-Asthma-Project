...
df_clean <- asthma_measures_df1[!is.na(asthma_measures_df1$asthma) & !is.na(asthma_measures_df1$ces_4_0_score), ]

# Calculate median asthma score per county for ordering
county_medians <- aggregate(asthma ~ county, df_clean, median)
county_order <- county_medians[order(county_medians$asthma), "county"]

# Create a long format version of the data for side by side plots
library(tidyr)
df_long <- pivot_longer(df_clean, 
                       cols = c("asthma", "ces_4_0_score"),
                       names_to = "measure",
                       values_to = "value")

# Make measure names more readable
df_long$measure <- factor(df_long$measure,
                         levels = c("asthma", "ces_4_0_score"),
                         labels = c("Asthma Rate", "CES 4.0 Score"))

# Create the comparison plots
library(ggplot2)

ggplot(df_long, aes(x = factor(county, levels = county_order), y = value)) +
  geom_boxplot(fill = "lightblue") +
  coord_flip() +
  facet_wrap(~measure, scales = "free_x") +
  labs(title = "Distribution of Asthma Rates and CES 4.0 Scores by County",
       subtitle = "Counties ordered by median Asthma Rate (ascending)",
       x = "County",
       y = "Value") +
  theme_minimal() +
  theme(
